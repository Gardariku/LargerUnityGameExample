...
namespace BrewedInk.WFC
{
    ...
    public class GenerationSpace
    {
        ...
        private bool TryGetRandomLowestEntropySlotWithOptions(out Slot slot)
        {
            var lowestSlots = new List<Slot>();
            var lowestEntropy = _totalWeightSum;
            slot = null;
            foreach (var kvp in _slotToPossibleModules)
            {
                if (kvp.Value.Count <= 1) continue;
            ...
            {
                if (kvp.Value.Count <= 1) continue;
                if (GetEntropy(kvp.Value) == lowestEntropy)
                    lowestSlots.Add(kvp.Key);
            }
            
            if (lowestSlots.Count > 0) slot = lowestSlots[Random.Next(0, lowestSlots.Count)];
            return slot != null;
        }
